...
import styles from "./AnimatedContentVariant2.module.css";
import { Link } from "react-router-dom";

const AnimatedContentVariant2 = () => {
    const textControls = useAnimation();
    const imageControls = useAnimation();
    const buttonControls = Array(4).fill(useAnimation()); // Массив контроллеров для кнопок

    // Состояния для хранения размеров шрифтов
    const [fontSizeSecond, setFontSizeSecond] = useState(208); // Размер шрифта для второго заголовка
    const fontSizeRatio = 87 / 208; // Соотношение размеров первого и второго заголовков (~0.418)

    useEffect(() => {
        const sequence = async () => {
            await textControls.start({ opacity: 1, y: 0, transition: { duration: 0.5 } });
            await imageControls.start({ opacity: 1, x: 0, transition: { duration: 0.6, ease: "easeOut" } });
    ...
    useEffect(() => {
        const updateScales = () => {
            ...
            const optimalFontSize = Math.min(optimalFontSizeWidth, optimalFontSizeHeight);
    ...
    return (
        <div className={styles.container}>
            <Link
                to="/"
                style={{ position: "absolute", top: 20, left: 20, color: "white", textDecoration: "none", zIndex: 1 }}
            >
                ← Назад
            </Link>
            <div className={styles.content}>
                ...
                {/* Див для заполнения оставшегося места */}
...
export default AnimatedContentVariant2;
